...
- `helper-projects/proto_mapi_emsmdb.py` — binary MAPI/HTTP emsmdb bağlayıcısı (`/mapi/emsmdb`): Connect → zincirli Execute (Logon → OpenFolder(Inbox) → GetContentsTable → SetColumns → QueryRows) ile MID oku → OpenMessage → GetPropertiesSpecific ile konu + Maildir'den çıkarılmış gövdeyi doğrula; MS-OXCROPS kablo düzeni (RPC_HEADER_EXT zincirli ROP buffer, FID 2-byte LE replid + 6-byte BE global counter) + Autodiscover'ın MAPI/OAB için Basic auth reklamı; uç erişilemezse zarif SKIP
