...
- Yüzey/işletim probe'ları:
  ...
  - `helper-projects/proto_mbck.py` (posta kutusu tutarlılık denetimi: `umailserver check mailbox` — throwaway hesaba import sonrası temiz, blob dizini silinince yetim girdileri tespit eder)
...
- `helper-projects/proto_mbck.py` — posta kutusu tutarlılık denetimi (`umailserver check mailbox`): throwaway bir hesap oluşturur, bilinen mbox'u import eder, denetimin TEMİZ olduğunu doğrular (üç kanonik store uyumlu = doğru semcore anahtarı, sahte ghost yok), sonra hesabın blob dizinini silip yetim girdilerin (orphan-index/identity) raporlandığını ve çıkış kodunun ≠0 olduğunu doğrular. Sunucu her durumda yeniden başlatılır.
