...
- `helper-projects/proto_jmap.py` — JMAP: session/capability, Mailbox/get, Email/query+get+set; gövde-decode yakınsaması: base64 HTML parçalı bir multipart/alternative mesaj SMTP ile teslim edilir, `Email/get` `fetchAllBodyValues` ile decode edilmiş `bodyValues` (ham base64 değil) + markup'sız tek-satır `preview` + textBody/htmlBody part'ları döner, fetch-flag'siz `Email/get` ise `bodyValues`'u hiç döndürmez (RFC 8621 §4.1.4 fetch-flag gate), `SearchSnippet/get` decode edilmiş snippet döner — paylaşılan `internal/mailbody` çıkarıcısı sayesinde JMAP, webmail görüntüleme/arama ve EAS ile aynı decode edilmiş içeriği okur
