...
- Protokol probe scriptleri (saf protokol istemcileri, `urllib`/`imaplib`/`poplib`/`smtplib` ile):
  ...
  - `helper-projects/proto_oof_internal_external.py` (Outlook iç/dış OOF: webmail API ile ayrı iç/dış gövde + audience kur → iç gönderen INTERNAL gövdeli yanıt alır (dış değil); audience=all iken dış gönderene yanıt kuyruğa girer; audience=internal iken dış gönderene yanıt YOK ama iç gönderene VAR; OOF'suz kullanıcı kendi Sieve vacation'ı etkilenmez)
  ...
  - `helper-projects/proto_mapi_nspi.py` (binary MS-OXNSPI adres defteri `/mapi/nspi`: Bind → QueryRows (GAL'i COLROW olarak listele, bob+alice SMTP adresiyle, her kayıtta display type, tek-satırlık sayfa tam bir satır döndürür — sayfalama onurlandırılır, tam GAL uncapped) → GetProps (bir kaydın özellikleri; kaydın taşımadığı özellik PtypErrorCode olarak döner) → GetMatches (PidTagAnr restriction'ını GAL üzerinde değerlendir — substring eşleşeni döner, eşleşmeyen boş) → GetTemplateInfo (özel template yok) → ResortRestriction (minid listesini display-name sırasına dizer) → SeekEntries (display-name sıralı GAL'de hedef ada lower-bound; cursor pozisyonu + satır, hedef tablo sonunu aşarsa not-found) → GetSpecialTable (GAL container hiyerarşisi; DT_CONTAINER permanent EntryID tel üzerinden çözülür) → ResolveNamesW (her ada resolved/ambiguous/unresolved durumu; yalnız tek-eşleşen ad satır döndürür) → DNToMId (sunucunun kendi permanent EntryID'sinden alınan DN → gerçek minid; bilinmeyen DN → 0) → GetPropList (bir kaydın mevcut prop tag'leri; bilinmeyen id hata) → UpdateStat (cursor'ı delta kadar ilerlet, tablo sonuna clamp) → CompareMinIds (iki minid'in tablo sırası + eşitlik) → QueryColumns (sunulan kolon seti) → ModProps/ModLinkAtt (read-only adres defteri yazımı ecNotSupported ile reddeder) → Unbind; tüm yanıtlar tel üzerinden çözülür + Basic-auth gate) + `proto_mapi_oab.py` (binary OAB `/mapi/oab`: oab.xml manifest → Full + template dosyalarının uzunluk + SHA-1 bütünlüğü manifest ile eşleşir → LZX deframe ile OAB v4 (0x20) kayıtları GAL alıcılarını bob+alice SMTP ile sıralar → HiddenFromGAL uçtan uca (hesap oluştur → OAB'de gör → gizle → düşer + seq ilerler) → bayat sequence 404; Basic-auth gate)
  - `helper-projects/proto_mapi_emsmdb.py` (binary MAPI/HTTP emsmdb bağlayıcısı `/mapi/emsmdb`: Connect → zincirli Execute(Logon → OpenFolder(Inbox) → GetContentsTable → SetColumns → QueryRows) ile MID oku → OpenMessage → GetPropertiesSpecific ile konu + Maildir'den çıkarılmış gövdeyi doğrula; MS-OXCROPS kablo düzeni (FID 2-byte LE replid + 6-byte BE global counter) + Autodiscover'ın MAPI/OAB için Basic auth reklamı; ayrıca RPC-over-HTTP (Outlook Anywhere) tüneli `/rpc/rpcproxy.dll`: bağımsız bir MS-RPCH istemcisiyle (probe bağımlılığı `impacket==0.12.0`) CONN/A1→A3→B1→C2 handshake → EMSMDB bind → EcDoConnectEx → EcDoRpcExt2(Logon) MAPI/HTTP ile aynı ROP dispatcher üzerinden başarılı; `mapi.ntlm_enabled` açıkken proxy HTTP-katmanı NTLM (MS-NLMP/RFC 4559) sunar ve impacket bunu otomatik müzakere eder (NEGOTIATE → 401'deki bağlantı-başına CHALLENGE → AUTHENTICATE), aksi halde Basic'e düşer; NTLM-özgü doğrulama: API login NT hash'i yakalar → ham NEGOTIATE'e proxy NTLMSSP CHALLENGE (type 2) ile yanıt verir → yanlış parola reddedilir (NT hash gerçekten doğrulanır, rubber-stamp değil); uç erişilemezse zarif SKIP)
  ...
  - `helper-projects/proto_autodiscover.py` (Autodiscover + Autoconfig) + `proto_autodiscover_ext.py` (EWS endpoint'ini taşıyan standart EXPR/ASUrl girdisi + MAPI/NSPI/OAB, devre dışı hesap 403)
  ...
  - `helper-projects/proto_ews_permissions.py` (EWS folder:PermissionSet = MAPI izin-biti modeli, RFC 4314 ACL üstüne: webmail ACL ile `lr` verilince EWS GetFolder(AllProperties) "Reviewer" gösterir; EWS UpdateFolder "Editor" yazınca webmail ACL maskesi `lrswipk` döner; owner için EffectiveRights dolu; public klasör PermissionSet okunur ama UpdateFolder yazma owner≠caller olduğundan AccessDenied)
  - `helper-projects/proto_eas.py` (Exchange ActiveSync 16.1 mobil senkron `/Microsoft-Server-ActiveSync`, bağımsız WBXML codec: OPTIONS sürüm/komut ilanı; Provision iki-aşamalı PolicyKey + provisioning gate 449; FolderSync; Email/Calendar/Contacts/Tasks down+up Sync + çapraz-yüzey yakınsama; MeetingResponse; ItemOperations; SendMail/SmartForward/SmartReply; Ping uzun-poll teslimde Status 2 + heartbeat + PIM ticker; remote wipe admin flag → 449 → Provision RemoteWipe → ack → partnership silinir; Settings dört alt-komut — DeviceInformation admin device list'e işler, DevicePassword, UserInformation alias tablosundan 16.1 Accounts, Oof kanonik OOF policy'sine yazar — webmail `/api/v1/vacation` ile yakınsar, disabled tarih taşısa da OofState 0; Search GAL store kanonik dizinden (NSPI/OAB ile aynı policy-filtreli kaynak) alıcı çözer — hesabın local-part'ıyla sorgu adresini DisplayName/EmailAddress/Alias ile döner; Search Mailbox store kanonik posta deposu üzerinde tam-metin sorgu çalıştırır (webmail aramasıyla aynı büyük/küçük-harf duyarsız substring eşleşme mantığını aynı mailstore üzerinde uygular) — teslim edilen mesajı bulur, opak LongId döner, ItemOperations Fetch-by-LongId ile mesajın tam gövdesini açar (round-trip), aynı token webmail `/api/v1/search`'te de aynı mesajı döndürür (çapraz-yüzey yakınsama probe ile doğrulanır); desteklenmeyen store (DocumentLibrary) Status 3)
...
- Yüzey/işletim probe'ları:
  ...
  - `helper-projects/proto_tnef_import.py` (TNEF (winmail.dat) içe aktarımı: `umailserver import --tnef` ile bağımsız bir Python TNEF yazıcısının ürettiği winmail.dat çözülüp kanonik depoya yazılır; konu/gövde/gönderen/Message-ID/ek sadık şekilde geri kurulur — Parse temel zarfı message property block'tan çıkarır —, deterministik MIME ile yeniden import idempotenttir, IMAP/JMAP/EWS çapraz görünürlük; TNEF round-trip'ini import yönünde tamamlar)
...
- `helper-projects/proto_mapi_nspi.py` — binary MS-OXNSPI adres defteri (`/mapi/nspi`): Bind (server GUID + sid çerezi), QueryRows (GAL'i COLROW olarak listele — bob+alice SMTP adresiyle, her kayıtta display type, tek-satırlık sayfa tam bir satır döndürür: sayfalama onurlandırılır, tam GAL uncapped — VAL-DIR-006 100 sınırı ad aramalarına ait), GetProps (bir kaydın özellikleri; kaydın taşımadığı özellik PtypErrorCode olarak döner), GetSpecialTable (GAL container hiyerarşisi — DT_CONTAINER permanent EntryID + container flags tel üzerinden çözülür), ResolveNamesW (ANR: her ada resolved/ambiguous/unresolved durumu, yalnız tek-eşleşen ad COLROW satırı döndürür — mids/satır asimetrisi), DNToMId (DN → minid eşlemesi; sunucunun kendi EntryID'sinden alınan DN gerçek minid'e, bilinmeyen DN 0'a döner), GetPropList (bir kaydın mevcut prop tag listesi, bilinmeyen id hata), UpdateStat (state-block cursor'ı delta kadar ilerletir, tablo sonuna clamp eder + gerçek hareketi döner), GetMatches (Restriction filtresini GAL üzerinde değerlendirir — AND/OR/NOT/PROPERTY[ANR substring]/EXIST; eşleşen kayıtların gerçek minid'leri + COLROW satırları, requested ile sınırlı), GetTemplateInfo (özel display template sunulmaz — istemci yerleşiğine düşer), ResortRestriction (istemci minid listesini display-name sırasına yeniden dizer), SeekEntries (GAL display-name'e göre sıralı; hedef ada lower-bound ile pozisyonlanır, cursor + tek satır döner, hedef sonu aşarsa not-found), CompareMinIds (iki minid'in tablo-sırası karşılaştırması + eşitlik), QueryColumns (sunulan kolon seti), ModProps/ModLinkAtt (read-only adres defteri yazımları ecNotSupported), Unbind (unbind-success), Basic-auth gate; tüm yanıtlar tel üzerinden çözülür
- `helper-projects/proto_mapi_oab.py` — binary OAB (MS-OXWOAB/MS-OXOAB, `/mapi/oab`): oab.xml manifest (Full + görüntü template'i; sürüm/boyut/SHA-1), Full + template dosyalarının uzunluk + SHA-1 bütünlüğü manifest ile eşleşir, LZX deframe (LZXD type-3) → OAB v4 (0x20) kayıtları GAL alıcılarını bob+alice SMTP ile sıralar, HiddenFromGAL uçtan uca (hesap oluştur → OAB'de gör → gizle → düşer + OAB sequence ilerler; içerik-anahtarlı yeniden derleme silinen alıcıyı da kapsar), bayat sequence 404, Basic-auth gate
- `helper-projects/proto_mapi_emsmdb.py` — binary MAPI/HTTP emsmdb bağlayıcısı (`/mapi/emsmdb`): Connect → zincirli Execute (Logon → OpenFolder(Inbox) → GetContentsTable → SetColumns → QueryRows) ile MID oku → OpenMessage → GetPropertiesSpecific ile konu + Maildir'den çıkarılmış gövdeyi doğrula; MS-OXCROPS kablo düzeni (RPC_HEADER_EXT zincirli ROP buffer, FID 2-byte LE replid + 6-byte BE global counter) + Autodiscover'ın MAPI/OAB için Basic auth reklamı; ayrıca RPC-over-HTTP (Outlook Anywhere) `/rpc/rpcproxy.dll`: bağımsız MS-RPCH istemcisiyle CONN handshake → EMSMDB bind → EcDoConnectEx → EcDoRpcExt2(Logon) aynı ROP dispatcher üzerinden; `mapi.ntlm_enabled` açıkken HTTP-katmanı NTLM (MS-NLMP/RFC 4559) — proxy ham NEGOTIATE'e bağlantı-başına NTLMSSP CHALLENGE (type 2) döner, doğru parola Logon'a ulaşır, yanlış parola reddedilir (stored NT hash gerçekten doğrulanır); uç erişilemezse zarif SKIP
...
- `helper-projects/proto_jmap.py` — JMAP: session/capability, Mailbox/get, Email/query+get+set; gövde-decode yakınsaması: base64 HTML parçalı bir multipart/alternative mesaj SMTP ile teslim edilir, `Email/get` `fetchAllBodyValues` ile decode edilmiş `bodyValues` (ham base64 değil) + markup'sız tek-satır `preview` + textBody/htmlBody part'ları döner, fetch-flag'siz `Email/get` ise `bodyValues`'u hiç döndürmez (RFC 8621 §4.1.4 fetch-flag gate), `SearchSnippet/get` decode edilmiş snippet döner — paylaşılan `internal/mailbody` çıkarıcısı sayesinde JMAP, webmail görüntüleme/arama, EAS, EWS ve MAPI ile aynı decode edilmiş içeriği okur
...
- `helper-projects/proto_quota_reconcile.py` — kota sayacı uzlaştırması: `QuotaUsed` yalnızca inbound teslimde (`IncrementQuota` rezervasyonu) tam tutulurdu; IMAP APPEND / EWS / JMAP yazımları onu artırmadan posta ekler, hiçbir silme yolu da azaltmazdı, dolayısıyla sayaç gerçek depo boyutundan sapardı (`mbsize` farkı raporlardı). Artık metadata store tek ekleme/silme chokepoint'inde (`StoreMessageMetadata` yeni mesajda, `DeleteMessage` kaldırmada) bir hook fire eder ve birleştirilmiş bir arka plan reconciler `QuotaUsed`'ı kanonik boyuta (saklanan mesaj boyutlarının toplamı, `mbsize`'ın raporladığı değer) `SetQuotaUsed` ile eşitler. Probe bir throwaway hesapta her iki sapma yönünü IMAP üzerinden kanıtlar: ~4 KB'lık bir APPEND `quota_used`'ı eklenen boyuta yükseltir (eskiden sayılmazdı), ikinci APPEND tekrar yükseltir, EXPUNGE ise tek mesaj boyutuna geri düşürür (eskiden azaltılmazdı). Aynı chokepoint EWS/JMAP yazımlarını da kapsadığından IMAP yüzeyi tüm yüzeyleri temsil eder. Hesap sonunda silinir
- `helper-projects/proto_publicfolders.py` — organizasyon geneli genel klasörler (domain başına paylaşılan ağaç, çapraz-protokol): `public_folders` admin config ile açılır; admin API ile `public@local.test` altında iki klasör kurulur — `Announcements` (`anyone:read` + bir poster'a `read+post`) ve `Staff` (yalnız poster'a `read+post`, `anyone` yok). Webmail keşfi (`/api/v1/public-folders`): reader (anyone üzerinden) Announcements'ı görür, Staff'ı görmez; poster ikisini de görür. IMAP "Public Folders/" paylaşılan ad-uzayı: poster ikisini LIST'ler, Announcements'a APPEND eder ve SELECT'te 1 mesaj görür; reader Announcements'ı LIST/SELECT eder ama APPEND'i `[NOPERM]` ile reddedilir (yalnız okuma) ve Staff'ı ne LIST'ler ne SELECT edebilir. EWS publicfoldersroot FindFolder aynı ACL-filtreli görünürlüğü yansıtır — admin'in kanonik storage'da oluşturduğu klasörler semcore identity'sine reconcile edilir (`reconcilePublicFolderIdentities`) ki kimlik-tabanlı EWS enumerasyonu IMAP/webmail ile aynı kaynakta yakınsasın. Kiracı izolasyonu: başka domaindeki kullanıcı webmail/IMAP/EWS'de local.test'in hiçbir genel klasörünü görmez (owner her yüzeyde caller'ın domaininden türetilir). `public_folders.enabled=false` iken klasörler her yüzeyde gizlenir. Throwaway hesaplar (`pf-*`), klasörler, ikinci domain temizlenir; config flag geri yüklenir
- `helper-projects/proto_search_folders.py` — MAPI/Outlook arama klasörleri (kalıcı kayıtlı-sorgu sanal klasörleri, çapraz-protokol): bir alıcının INBOX'ına iki ileti teslim edilir (biri "boss" gönderenden, biri "peer" gönderenden, gövdesinde ayırt edici bir token taşıyan). EWS CreateFolder `<t:SearchFolder>` (From contains boss, base = inbox) → FindFolder klasörü adıyla listeler; FindItem yalnız boss iletisini döndürür, peer'i From kriteriyle dışlar. İkinci bir search folder (Body contains token, base = inbox) → FindItem peer iletisini gövde-metni eşleşmesiyle döndürür, boss'u dışlar (gövde okuma `collectMailItems`'ta tam gövdeyle yapılır). Üçüncü bir search folder (From contains boss, base = Sent) → FindItem hiçbir şey döndürmez: eşleşen ileti INBOX'ta, kapsam onu dışlar. DeleteFolder klasörü kaldırır ve FindFolder artık listelemez. Webmail `/api/v1/search-folders`: oluştur (201) → `/results` boss iletisini döndürür (peer değil) → kriteri gövde-token'ına PUT ile güncelle → `/results` peer iletisine geçer → DELETE kaldırır. Her iki yüzey de aynı kanonik `semcore.SearchFolderDef`'i okur (EWS restriction değerlendiricisi vs webmail depo taraması; karar tek `Matches`). Throwaway hesap (`sf-rcpt-*`) silinir
...
- `helper-projects/proto_oof_internal_external.py` — Outlook iç/dış ofis-dışı (OOF) teslim davranışı: bir alıcının OOF'u webmail API ile ayrı iç gövde (`message`) + dış gövde (`external_message`) + `audience` ile kurulur ve GET round-trip doğrulanır. (1) Yerel-domain (internal) bir gönderen alıcıya posta atınca otomatik yanıtı KENDİ kutusunda INTERNAL gövdeyle alır (EXTERNAL gövde DEĞİL — gövde seçimi). (2) `audience=all` iken dış-domain (non-hosted) bir gönderenden gelen postaya otomatik yanıt outbound kuyruğa girer (admin `/api/v1/admin/queue`'da from=alıcı, to=dış-gönderen). (3) `audience=internal` iken aynı dış gönderene yanıt YOK (kuyrukta girdi yok) ama iç gönderen hâlâ yanıt alır — audience zorlaması. Dış gövdenin tam içeriği `internal/ews` `TestOOFReplyFor` birim testiyle kanıtlanır (kuyruk yalnız zarf from/to gösterir). OOF policy'si olmayan kullanıcının kendi ManageSieve vacation'ı etkilenmez (regresyon proto_sieve_exec'te). Throwaway hesaplar (oof-*) silinir
...
- `helper-projects/proto_autodiscover_ext.py` — Autodiscover derinliği: EWS endpoint'ini taşıyan standart EXPR/ASUrl girdisi + MAPI/NSPI/OAB, devre dışı hesap 403, autoconfig auth/socket alanları
...
- `helper-projects/proto_ews_permissions.py` — EWS klasör izinleri (MAPI izin-biti modeli, kanonik RFC 4314 ACL üstüne): owner webmail ACL API ile bir kullanıcıya INBOX üzerinde `lr` verir → EWS GetFolder(inbox, AllProperties) o grant'ı PermissionSet'te PermissionLevel "Reviewer" olarak gösterir; aynı çağrı owner için EffectiveRights döndürür (owner tüm hakları örtük tutar — Read/Modify/Delete/CreateContents/CreateHierarchy). EWS UpdateFolder ile folder:PermissionSet "Editor" yazılır → webmail ACL API geri okunduğunda delegate'in hak maskesi `lrswipk` (Editor preset) olur — çapraz-yüzey yazma round-trip. İzin düzenleme owner-only self-service: bir domain kullanıcısı public klasörün PermissionSet'ini OKUR (FindFolder publicfoldersroot, AllProperties) ama aynı klasöre UpdateFolder yazma owner≠caller olduğundan `ErrorAccessDenied` ile reddedilir (public ACL admin-only kalır). Throwaway hesaplar/public klasör/grant'lar temizlenir, public_folders flag geri yüklenir
- `helper-projects/proto_eas.py` — Exchange ActiveSync (EAS 16.1) mobil senkronizasyon (`/Microsoft-Server-ActiveSync`), bağımsız bir Python WBXML codec ile (sunucu codec'iyle paylaşılmaz; token tabloları MS-ASWBXML spec'inden ayrı transkripsiyon): OPTIONS 14.1/16.0/16.1 sürüm + komut seti ilanı; Provision iki-aşamalı PolicyKey handshake (geçici anahtar + EASProvisionDoc → onaylanınca kalıcı anahtar); provisioning gate (PolicyKey'siz / bayat / `0` sentinel ile gelen her komut HTTP 449 "Retry after Provision" alır); FolderSync kanonik klasör seti (mail klasörleri + lazy-ensure edilen Calendar/Contacts/Tasks collab klasörleri); dört veri sınıfında Sync down+up — Email (AirSyncBase gövde + okundu/flag/taşı/sil), Calendar (icaltz TZ), Contacts (vCard), Tasks (VTODO) — her biri kanonik store'a yazılır; çapraz-yüzey yakınsama (REST/webmail ile oluşturulan etkinlik/görev/kişi EAS'ta görünür ve EAS yazımları REST'te görünür); MeetingResponse (accept/tentative/decline → kanonik takvim); ItemOperations Fetch; SendMail/SmartForward/SmartReply (paylaşılan Sieve-farkında submit + Sent'e dosyalama); Ping uzun-poll (teslimde anlık Status 2 + değişen INBOX collection'ı adlandırır, ~30s API WriteTimeout'unu aşan held — write-deadline kaldırma, heartbeat expiry'de Status 1, saf calendar collection'da PIM ticker ile etkinlik değişiminde Status 2); remote wipe (admin flag → device sonraki komutta 449 → Provision RemoteWipe directive → device ack → partnership silinir, dönen device sıfırdan provision olur); Settings dört alt-komut — DeviceInformation Set client'ın raporladığı device kimliğini (Model/IMEI/FriendlyName/OS/OSLanguage/PhoneNumber) partnership'e işler ve admin device list'te görünür; DevicePassword Set acknowledge (DevicePassword>Set>Status); UserInformation Get hesabın SMTP adreslerini kanonik alias tablosundan 16.1 Accounts>Account>EmailAddresses şeklinde döner; Oof Get/Set kanonik OOF policy'sine (webmail/EWS/JMAP'in paylaştığı semcore policy store + Sieve recompile) yazar/okur — EAS Oof Set sonrası webmail `/api/v1/vacation` GET aynı enabled durumu + iç gövde (`message`) + dış gövde (`external_message`) gösterir (çapraz-yüzey OOF convergence), disabled policy bir schedule penceresi taşısa da OofState 0 döner; Search komutu iki store: GAL store — kanonik dizinden (binary NSPI + OAB adres defteriyle aynı policy-filtreli kaynak, `mapi.ResolveGAL`) alıcı çözümleme: probe hesabın kendi local-part'ıyla sorgu yapar ve adresini Result>Properties altında DisplayName/EmailAddress/Alias ile geri alır (Alias adres local-part'ı, adapter türetimini uçtan uca kanıtlar), Range/Total tek eşleşme penceresini raporlar; Mailbox store — kanonik posta deposu (webmail aramasının taradığı aynı mailstore) üzerinde, webmail `/api/v1/search` ile AYNI büyük/küçük-harf duyarsız substring eşleşme mantığını gönderen + alıcı + konu + gövde üzerinde çalıştırır: probe bir mesajın gövdesine benzersiz bir token yerleştirip teslim eder → token ile sorgulayınca eşleşen mesaj opak LongId taşıyan bir Result olarak döner → bu LongId ItemOperations Fetch (Store=Mailbox) ile geri beslenir ve mesajın tam, kesilmemiş gövdesini (token dahil) açar (round-trip: bir istemcinin açamayacağı sonuç işe yaramaz; açma yolu uçtan uca kanıtlanır) → aynı token webmail `/api/v1/search`'te de aynı mesajı döndürür (çapraz-yüzey yakınsama probe ile doğrulanır — telefon araması = tarayıcı araması; gövde birleştirmesinden sonra EAS ve webmail araması ikisi de gövdeyi paylaşılan `internal/mailbody` ile decode edip tarar, dolayısıyla başlık + düz-metin + encoded/multipart gövdede yakınsarlar), Range emit edilen satırları + Total tam eşleşme sayısını raporlar; desteklenmeyen store (DocumentLibrary) yalnızca Status 3 döner (Response bloğu yok); ayrıca gövde-decode yakınsaması: base64-kodlanmış HTML parçası taşıyan bir multipart/alternative mesaj teslim edilir ve webmail mail API'sinin (`/api/v1/mail/inbox`) onu ham base64 yerine decode edilmiş HTML gövdesiyle + markup'sız temiz metin preview'iyle döndürdüğü doğrulanır — paylaşılan `internal/mailbody` çıkarıcısı sayesinde webmail görüntüleme/arama, EAS, JMAP, EWS ve MAPI aynı decode edilmiş içeriği okur (XSS yüzeyi değişmez; webmail body'yi zaten DOMPurify ile sanitize eder). Tümü provisioning gate + Basic-auth gate arkasında; throwaway hesap (`eas-*`) + ayrı throwaway wipe device kullanır
...
- `helper-projects/proto_tnef_import.py` — TNEF (winmail.dat) içe aktarımı: BAĞIMSIZ bir Python TNEF yazıcısı (MS-OXTNEF framing + bir MAPI property block, Go decoder'ından ayrı) ile gerçek bir winmail.dat üretir (PR_SUBJECT/PR_SENDER/PR_INTERNET_MESSAGE_ID/PR_BODY + bir ek), böylece import çapraz-implementasyon olarak `internal/tnef.Parse`'ı başka bir yazıcının ürettiği TNEF baytlarına karşı doğrular; .tnef'i mount'lu veri dizinine yazar, sunucuyu durdurur (bbolt tek-yazıcı), `umailserver import --tnef` çalıştırır (idempotensi için iki kez — deterministik MIME boundary sayesinde ikinci çalıştırma duplicate olarak atlanır), sunucuyu yeniden başlatır, sonra içe aktarılan mesajın IMAP + JMAP + EWS'de göründüğünü VE içeriğinin sadık olduğunu (gövde, ek dosya adı, gönderen From'u, Message-ID) doğrular. Gelen TNEF decode (`internal/tnef.Parse`) ve giden `export --tnef` zaten vardı; bu, TNEF round-trip'ini import yönünde kapatır. Sunucu her durumda yeniden başlatılır.
