...
- Yüzey/işletim probe'ları:
  ...
  - `helper-projects/proto_mbck.py` (posta kutusu tutarlılık denetimi + onarımı: `umailserver check mailbox [--repair]` — throwaway hesaba import sonrası temiz, blob dizini silinince yetim girdileri tespit eder, `--repair` ile yetim index/identity girdilerini siler ve yeniden denetimin temiz olduğunu doğrular)
...
- `helper-projects/proto_mbck.py` — posta kutusu tutarlılık denetimi ve onarımı (`umailserver check mailbox [--repair]`): throwaway bir hesap oluşturur, bilinen mbox'u import eder, denetimin TEMİZ olduğunu doğrular (üç kanonik store uyumlu = doğru semcore anahtarı, sahte ghost yok), sonra hesabın blob dizinini silip yetim girdilerin (orphan-index/identity) raporlandığını ve çıkış kodunun ≠0 olduğunu doğrular. Ardından `--repair` ile yetim IMAP index ve semcore identity girdilerinin silindiğini, çıkış kodunun 0 olduğunu ve sonraki sade denetimin yeniden TEMİZ döndüğünü doğrular. (EWS-ghost RECREATE yolu `internal/mailcheck` unit testleriyle kapsanır: tek küresel semcore identity.db tek bir hesap için cerrahi olarak temizlenemez — her dev posta kutusunu silmeden — bu yüzden probe'ta tohumlanmaz.) Sunucu her durumda yeniden başlatılır.
