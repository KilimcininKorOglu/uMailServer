...
func cmdQuickstart(args []string) {
	...
	fs.StringVar(&password, "password", "", "Admin password (skips interactive prompt)")
	fs.BoolVar(&skipConfig, "skip-config", false, "Skip config file generation (domain/account/DKIM only)")
	_ = fs.Parse(args)
	...
	// Generate config (unless skipped)
	var publicKey []byte
	var err error
	if !skipConfig {
